...
for evt in decode_table:
    rows.append({
        "name": evt.key,
        "cuda_time_ms": evt.cuda_time  / 1000,   # microseconds → ms
        "cpu_time_ms":  evt.cpu_time_total  / 1000,
        "cuda_pct":     evt.cuda_time  / max(sum(e.cuda_time  for e in decode_table), 1) * 100,
        "calls":        evt.count,
...
for evt in prefill_table:
    rows.append({
        "name": evt.key,
        "cuda_time_ms": evt.cuda_time  / 1000,
        "cpu_time_ms":  evt.cpu_time_total  / 1000,
        "cuda_pct":     evt.cuda_time  / max(sum(e.cuda_time  for e in prefill_table), 1) * 100,
        "calls":        evt.count,
        "input_shapes": str(evt.input_shapes) if hasattr(evt, "input_shapes") else "",
    })
